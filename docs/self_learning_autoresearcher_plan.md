...
6. ~~Implement M2 + M3.~~ **Done 2026-07-02: M2 bug book (deterministic error→fix mining from journal debug chains via `python -m experience.ingest --with-bugbook`, injected into the debug agent on error match), M2 lessons (`python -m experience.reflect` — LLM distillation of run journals into scoped, confidence-tagged lessons, injected alongside episodic records), M3 solution library (`--with-solutions` — top-N pipeline indexing, top-1 injected into draft as an adapt-don't-copy reference). Per-mechanism config switches `agent.experience.use_{episodic,lessons,bugbook,solutions}` for E4; leakage guard and snapshot hash cover all four corpora; 10 tests green.**
7. ~~Placebo-store builder + E1 operational prep.~~ **Done 2026-07-02: `python -m experience.placebo` (arm C — per-corpus fixed-point-free content derangement preserving sizes/tokens/labels/provenance, deterministic under --seed, auditable via placebo_meta.json) and `experience/E1_RUNBOOK.md` (gates, per-arm overrides, store-build pipeline, canary checks, failure modes). E0 status: phoenix landed grading-in-worker + AIS switch fixes and launched corrected Tranche 1R (144 runs, $250 cap) on 2026-07-02 — but the anchor is still uncommitted there.**
8. Blocked until phoenix E0 completes: anchor freeze commit (`ANCHOR` in the runbook), Tranche 1R base-system confirmation, then E1 step 1 (arm-A runs). Arm D's foreign store is built after the B stores exist (token-count matching). CPU caveat from phoenix: denoising is torch.hub-bound and often invalid on CPU workers (arm-symmetric; handled by the flag/sensitivity analysis).
